program main
  ...
  write(comando,'(4A)') "grep -i 'EXPORT R =' ", trim(outputfile), " >> ", radiusfile
  ...
  write(comando,'(4A)') "grep -i -n 'Molecular Polarizability' ", trim(outputfile), " >> ", polfile
  ...
  do jk=1,lineas
     ...
     !! skip the 6 energies computed by loprop
     do ij=1,6
        read(125,*) auxchprev
     end do
     ...
     !! skip the 7 dipole moments computed by loprop
     do ij=1,6
        read(126,*) auxchprev
     end do
     ...
     !!first row
     write(comando,*) "head -n ", lineaspol+3, trim(outputfile), "| tail -1 >", polxyzfile
     CALL EXECUTE_COMMAND_LINE(comando,wait=.true.)

     !!second row
     write(comando,*) "head -n ", lineaspol+4, trim(outputfile), "| tail -1 >>", polxyzfile
     CALL EXECUTE_COMMAND_LINE(comando,wait=.true.)

     !!third row
     write(comando,*) "head -n ", lineaspol+5, trim(outputfile), "| tail -1 >>", polxyzfile
     CALL EXECUTE_COMMAND_LINE(comando,wait=.true.)
  ...
  write(130,'(12A16)') '# R', 'E (hartree)', 'mu_x (D)','mu_y (D)','mu_z (D)','mu_t (D)', &
       'alphaxx (a.u.)','alphayy (a.u.)','alphazz (a.u.)'
  
  Do jk=1,lineas
     write(130,'(10ES16.6E2)') r(jk), energy(jk), (dipole(ij,jk), ij=1,4), &
          (polarizabilities(ij,jk), ij=1,3)
  End Do
